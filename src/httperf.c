...
static struct option longopts[] =
  {
    ...
    {"add-random-header",   required_argument, (int *) &param.random_additional_header, 0},
    ...
    {"server-name",  required_argument, (int *) &tmp_server_name,	0},
    ...
    {"random-uri",   no_argument,       &param.random_uri,              0},
    {"random-extension",   required_argument,  (int *) &param.random_extension,0},
    ...
    {"client-name",  required_argument, (int *) &param.clientname,		0},
    {"client-port",  required_argument, (int *) &param.clientport,		0},
    {"no-ua-hdr",	 no_argument, (int *) &param.no_ua_hdr,		1},
    {0,		     0,			0,				0}
  };
...
usage (void)
{
  printf ("Usage: %s "
	  ...
	  "\t[--num-calls N] [--num-conns N] [--precentiles]\n"
          "\t[--period [d|u|e]T1[,T2]] [--port N] [--print-reply\n"
          "\t[header|body]] [--print-request [header|body]]\n"
	  "\t[--rate X] [--recv-buffer N] [--retry-on-failure] "
	  "[--send-buffer N]\n"
	  "\t[--server S] [--server-name S] [--session-cookies] [--summary]\n"
#ifdef HAVE_SSL
	  ...
	  "\t[--wset N,X] [--client-name S] [--client-port N] [--no-ua-hdr]\n",
	  prog_name);
}
...
main (int argc, char **argv)
{
  ...
  while ((ch = getopt_long (argc, argv, "d:hvV", longopts, &longindex)) >= 0)
	...
	case 0:
	  ...
	  if (flag == &param.method)
	  ...
	  else if (flag == &param.random_additional_header)
	    param.random_additional_header = optarg;
	    ...
	    param.server = optarg;
	  else if (flag == &tmp_server_name && param.num_server_names < 100) 
            param.server_names[param.num_server_names++] = optarg;
	    ...
	    param.uri = optarg;
	  else if (flag == &param.random_uri)
	    param.random_uri = 1;
	  else if (flag == &param.random_extension) {
              param.random_extension = strtoul (optarg, &end, 10);
	      if (errno == ERANGE ||
                  end == optarg ||
                  *end ||
                  param.random_extension < 1 ||
                  param.random_extension > 999)
		{
		  fprintf (stderr, "%s: illegal --random-extension param %s\n",
			   prog_name, optarg);
		  exit (1);
		}
          }
	  ...
	  else if (flag == &param.clientname)
	    {
	      param.clientname = optarg;
		}
	  else if (flag == &param.clientport)
	    {
	      errno = 0;
	      param.clientport = strtoul (optarg, &end, 10);
	      if (errno == ERANGE || end == optarg || *end
		  || (unsigned) param.clientport > 0xffff)
		{
		  fprintf (stderr, "%s: illegal port number %s\n",
			   prog_name, optarg);
		  exit (1);
 ...
 if (param.additional_header || param.random_additional_header || param.method)
    gen[num_gen++] = &misc;
  ...
  if (!summary) {
    ...
    if (percentiles) printf (" --percentiles");
    ...
    if (param.num_server_names) {
        int i;
        for (i = 0; i < param.num_server_names; i++)
            printf(" --server-name=%s",param.server_names[i]);
    }
    if (param.port) printf (" --port=%d", param.port);
    if (param.uri) printf (" --uri=%s", param.uri);
    if (param.random_uri) printf (" --random-uri");
    if (param.random_extension) printf (" --random-extension=%d",
                                        param.random_extension);
    ...
    if (param.random_additional_header)
      printf (" --add-random-header='%s'", param.random_additional_header);
    ...
    if (param.clientname) printf(" --client-name=%s", param.clientname);
	if (param.clientport >= 0) printf(" --client-port=%d", param.clientport);
  }
  else {/*summary; much less output*/
    char fullurl[50];
    snprintf(fullurl,sizeof(fullurl)-1,"%s:%d%s",param.server,param.port,
		param.uri);
    /*printf("%-44s",fullurl);*/
    printf("%s",fullurl);
  }
