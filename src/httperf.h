...
typedef struct Cmdline_Params
  {
    ...
    const char *server_names[100]; /* list of server names */
    int num_server_names;
    int next_server_name; /* index of next server name to choose */
    int port;		/* (default) server port */
    char *uri;	/* (default) uri */
    int random_uri;      /* replace $ in uri with random digits */
    int random_extension; /* add .html with this percentage, else add .gif */
    ...
    const char *random_additional_header;	/* additional headers, add 50% of the time */
    struct
      {
	u_int id;
	u_int num_clients;
